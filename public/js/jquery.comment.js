(function($){
	function crateCommentInfo(obj){
		...
		el = el + "</div><div class='col-md-2'><span class='reply-btn'>Reply</span></div></div></div><div class='reply-list'>";
	...
	function createReplyComment(reply){
		var replyEl = "<div class='reply'><div><a href='javascript:void(0)' class='replyname'>"+reply.replyName+"</a>:<a href='javascript:void(0)'>@"+reply.beReplyName+"</a><span>"+reply.content+"</span></div>"
						+ "<p><span>"+reply.time+"</span> <span class='reply-list-btn'>Reply</span></p></div>";
		return replyEl;
	}
	...
	function replyClick(el){
		el.parent().parent().append("<div class='replybox'><textarea cols='80' rows='50' placeholder='Comment......' class='mytextarea' ></textarea><span class='send'>Submit</span></div>")
		.find(".send").click(function(){
			...
			if(content != ""){
				...
				$(".replybox").remove();
				parentEl.find(".reply-list").append(replyString).find(".reply-list-btn:last").click(function(){alert("You can not reply yourself");});
			}else{
				alert("nothing");
			}
		});
